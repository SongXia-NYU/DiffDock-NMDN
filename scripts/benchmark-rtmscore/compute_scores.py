...
def merck_rank():
    ...
    rank_info_by_score, count_info_by_target = MerckFEPScoreCalculator.compute_rank_info(out_df, ["rtm_score"])
...
def casf_screen():
    ...
    for target_name, df in score_df.groupby("tgt_pdb"):
        ...
        df_out = df[["#code_ligand_num", "rtm_score"]].rename({"rtm_score": "score"}, axis=1)
