...
kano_root: str = osp.abspath(osp.join(osp.dirname(osp.dirname(osp.dirname(__file__))), "kano"))

ele2emb = pickle.load(open(osp.join(kano_root, 'initial/ele2emb.pkl'),'rb'))
def ele_features(ele):
    fele = ele2emb[ele]
    return fele.tolist()

rel2emb = pickle.load(open(osp.join(kano_root, 'initial/rel2emb.pkl'),'rb'))
def relation_features(e1,e2):
    frel = rel2emb[(e1,e2)]
    return frel.tolist()

with open(osp.join(kano_root, '../chemprop_kano/data/funcgroup.txt'), "r") as f:
...
fg2emb = pickle.load(open(osp.join(kano_root, 'initial/fg2emb.pkl'), 'rb'))
def match_fg(mol):
    fg_emb = [[1] * 133]
    pad_fg = [[0] * 133]
    for sm in smart:
...
class MolGraph:
    ...
    def __init__(self, smiles: Optional[str], mol: Optional[rdkit.Chem.rdchem.Mol], args: Namespace, prompt: bool):
        """
        ...
        mol = Chem.MolFromSmiles(smiles) if mol is None else mol
...
class BatchMolGraph:
    ...
    def __init__(self, mol_graphs, args: Namespace):
        ...
        self.f_atoms = torch.FloatTensor(f_atoms).type(floating_type)
        self.f_bonds = torch.FloatTensor(f_bonds).type(floating_type)
        self.f_fgs = torch.FloatTensor(f_fgs).type(floating_type)
    ...
    def get_components(self) -> Tuple[torch.Tensor, torch.Tensor,
