...
def pairwise_dist(X: torch.Tensor, Y: torch.Tensor):
    ...
    Y = Y.double().unsqueeze(0)
    
    dists = -2 * torch.bmm(X, Y.permute(0, 2, 1)) + torch.sum(Y**2,    axis=-1).unsqueeze(1) + torch.sum(X**2, axis=-1).unsqueeze(-1)
    dists = torch.nan_to_num((dists**0.5),10000).squeeze(0)
    return dists.type(floating_type)
...
def bn_edge_calculate_fast(mol_dict: dict):
    """
    A faster implementation to calculate the edge based on cutoff (10 A). 
    Input dictionary should contain key "R" for all coordinates of shape (-1, 3) and "Z" for atomic numbers of shape (-1).
    """
    assert "R" in mol_dict, mol_dict.keys()
    assert "Z" in mol_dict, mol_dict.keys()
    pair_dist = pairwise_dist(mol_dict["R"], mol_dict["R"])
    ...
    mol_dict["BN_edge_index"] = edge_index.T
    mol_dict["num_BN_edge_index"] = torch.as_tensor([edge_index.shape[0]])
    mol_dict["atom_mol_batch"] = torch.zeros_like(mol_dict["Z"])
    return mol_dict
